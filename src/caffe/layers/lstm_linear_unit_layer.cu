...
template <typename Dtype>
void LSTMLinearUnitLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
...
__global__ void LSTMActsBackward(const int nthreads, const int dim,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    if (d < 3 * dim) {
      ...
      X_diff[index] = X_acts_diff[index];
    }
  }
}

template <typename Dtype>
void LSTMLinearUnitLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(LSTMLinearUnitLayer);
