...
template <typename Dtype>
void LSTMLinearUnitLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void LSTMLinearUnitLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  for (int n = 0; n < num; ++n) {
    for (int d = 0; d < hidden_dim_; ++d) {
      ...
      const Dtype g = X[3 * hidden_dim_ + d];
...
template <typename Dtype>
void LSTMLinearUnitLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(LSTMLinearUnitLayer);
#endif

INSTANTIATE_CLASS(LSTMLinearUnitLayer);
REGISTER_LAYER_CLASS(LSTMLinearUnit);
