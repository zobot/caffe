...
class EntropyLossLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  ...
  Dtype EntropyLossReference(const int count, const int num,
                                         const Dtype* input,
                                         const Dtype* target) {
    Dtype loss = 0;
  ...
  void TestForward() {
    ...
    for (int i = 0; i < 100; ++i) {
      ...
      Dtype reference_loss = kLossWeight * EntropyLossReference(
...
/*
TYPED_TEST(EntropyLossLayerTest, TestEntropyLoss) {
  this->TestForward();
}
*/
